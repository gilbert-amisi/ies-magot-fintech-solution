<template>
  ...
  export default {
    props: ['idSubscription', 'start', 'end', 'level', 'maxnumber'],
    data () {
      const addSubscription = () => {
      var form = new FormData()
      form.append('subscriptionId', this.idSubscription)
      ...
      if (this.$maxnumber !== null && this.start !== null && this.end !== null && this.start <= this.end  && this.level !== null) {
          axios.post(this.$urlServer + 'subscriptionControllers/updateSubscription.php', form, {
          }).then((res) => {
            this.$q.dialog({
              title: 'Success',
              message: ('Client subscription successfully updated'),
              ok: {
                push: true
              },
              cancel: {
