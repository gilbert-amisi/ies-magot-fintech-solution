...
$data = MagoConnection::connect()->query("SELECT mb.educationLevel, COUNT(mb.id) AS nbMember
FROM member mb LEFT JOIN vsla vs ON mb.vslaId = vs.id
WHERE vs.id = '{$vslaId}' AND mb.`position` <> 'Ordinary member'
GROUP BY mb.educationLevel");

while ($res = $data->fetch()) {
    $donnees = array_merge($donnees, array($res['nbMember']));
    $entete = array_merge($entete, array($res['educationLevel']));
}
